...
const HistoryTrade = () => {
  ...
  const getTradeList = async () => {
    const apiUrl = URLHelper.ALL_TRADE_HISTORY;
    ...
    try {
      ...
      console.log(
        'Response length Trade HISTORY ::',
        response.data.data.items.data,
      );
      console.log('Response :', response.data.data.items.data.length);

      setDepositList(response.data.data.items.data);
  ...
  const getWithdrawList = async () => {
    const apiUrl = URLHelper.TRADE_HISTORY;
    ...
    try {
      ...
      console.log(
        'Response length Transfer HISTORY ::',
        response.data.data.items.data,
      );
      console.log('Response :', response.data.data.items.data.length);

      const depositData = response.data.data.items.data;
    ...
    <View
      ...
      {filteredData.length > 0 ? (
        <FlatList
          ...
          renderItem={({item}) => {
            return <WithdrawTradeItem value={item} />;
          }}
          ListFooterComponent={
            <View
              style={{
